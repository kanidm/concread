...
impl<K: Clone + Ord + Debug, V: Clone> Node<K, V> {
    pub(crate) fn new_leaf(txid: u64) -> Self {
        Node {
            #[cfg(test)]
            nid: alloc_nid(),
            txid,
    ...
    pub(crate) fn new_ableaf(txid: u64) -> ABNode<K, V> {
        Arc::new(Self::new_leaf(txid))
    }

    pub(crate) fn new_branch(txid: u64, l: ABNode<K, V>, r: ABNode<K, V>) -> ABNode<K, V> {
        Arc::new(Node {
            #[cfg(test)]
            nid: alloc_nid(),
            txid,
    ...
    pub(crate) fn new_leaf_ins(txid: u64, k: K, v: V) -> ABNode<K, V> {
    ...
    pub(crate) fn req_clone(&self, txid: u64) -> ABNode<K, V> {
...
impl<K: Clone + Ord + Debug, V: Clone> Branch<K, V> {
    pub fn new(left: ABNode<K, V>, right: ABNode<K, V>) -> Self {
        ...
        let kptr: *mut K = new.key[0].as_mut_ptr();
        let lptr: *mut ABNode<K, V> = new.node[0].as_mut_ptr();
        let rptr: *mut ABNode<K, V> = new.node[1].as_mut_ptr();
    ...
    */
    ...
    pub(crate) fn clone_sibling_idx(&mut self, txid: u64, idx: usize) -> usize {
    ...
    fn clone_idx(&mut self, txid: u64, idx: usize) {
