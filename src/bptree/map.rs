//! See the documentation for `BtreeMap` module.
...
impl<'a, K: Clone + Ord + Debug, V: Clone> BptreeMapWriteTxn<'a, K, V> {
    ...
    /// Get the current transaction id. You should not rely on the order of transaction id's only
    /// their equality.
...
#[cfg(test)]
mod tests {
    use super::super::node::{assert_released, L_CAPACITY};
    use super::BptreeMap;
    // use rand::prelude::*;
